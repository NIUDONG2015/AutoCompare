	...
	public void doMatch(){
		...
		for (int i = 0; i < standardWell.getBigLayers().size(); ++i) {
			...
			if(bigLayer.getSmallLayers().size()>0){
				...
				bigLayer.getSmallLayers().get(bigLayer.getSmallLayers().size()-1).setMatchResName(bigLayer.getSmallLayers().get(bigLayer.getSmallLayers().size()-1).getName());
			}
		}
		...
		/*for (int i = 0; i < wellList.size(); ++i) {
			...
			for (int j = 0; j < well.getBigLayers().size(); ++j) {
				...
				for (int k = 0; k < bigLayer.getSmallLayers().size(); ++k) {
					SmallLayer smallLayer = bigLayer.getSmallLayers().get(k);
					System.out.println("			层位:" + smallLayer.getName() + "  归一化：" + smallLayer.getNor()+
							"  匹配结果:"+smallLayer.getMatchResName());
				}
				System.out.println("");
			}
			System.out.println("");
